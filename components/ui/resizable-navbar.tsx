...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  const scrollToId = useCallback((link: string) => {
    if (!link) {
    ...
    if (link.startsWith('#')) {
      ...
      if (element) {
        ...
        const scrollPosition = elementTop + window.pageYOffset - 80;
        window.scrollTo({
          top: scrollPosition,
          behavior: 'smooth'
        });
      ...
      // For other page links, you might want to navigate
      window.scrollTo({ top: 0, behavior: 'smooth' });
  ...
  const handleClick = (link: string) => {
    // First call the onItemClick callback if provided
    if (onItemClick) {
      onItemClick();
    }
    ...
    >
      {items.map((item, idx) => (
        <Button
          ...
          className=" bg-transparent hover:bg-transparent relative px-4 py-2 text-neutral-600 dark:text-neutral-300"
          key={`link-${idx}`}
        >
          {hovered === idx && (
            <motion.div
