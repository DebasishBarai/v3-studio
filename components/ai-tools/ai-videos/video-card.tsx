  ...
  return (
    <Link href={`/ai-tools/ai-video/editor/${video._id}`}>
      <motion.div
        ...
        className="relative rounded-xl overflow-hidden cursor-pointer border border-gray-200 dark:border-gray-700 shadow-sm bg-slate-100 dark:bg-slate-800/10"
      >
        {/* Scene Image */}
        {sceneImages?.length ? (
          <Image
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 flex items-center justify-between">
          ...
          <div
            ...
            {video?.videoUrl && (
              <Link
                href={cdn(video.videoUrl)}
                target="_blank">
                <Button size="sm" className="bg-white/20 hover:bg-white/30 text-white border-none z-50">
